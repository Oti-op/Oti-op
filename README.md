# Hi, I'm Oti 👋  

I'm a Computer Science (AI) student

### 🧠 About Me :)

```csharp
public class Oti {
    private int age = 18;
    public string[] skills = { "Python", "JavaScript", "C++", "HTML", "CSS" };
    public string[] experiences = { "AI and Cyber risk research @ E3 Consulting", "Chair @ UoP CompSoc", "Technology Intern @ Oscom", "SRC President @ Westerfield College, mentee @ Challa.com" };
    public string[] hobbies = { "public speaking", "coding", "debate", "reading", "watching youtube!" }

    public void learn() {
        Console.WriteLine("Always learning how tech can drive sustainable change and innovation.");
    }
}
